...
readonly TERRA_CLI_VERSION="0.347.0"

readonly CLI_SERVER_NAME_TAG_KEY="CliServerName"
readonly TERRA_WORKSPACE_ID_KEY="WorkspaceId"

readonly JUPYTER_USER_NAME="ec2-user"
readonly JUPYTER_USER_HOME="/home/${JUPYTER_USER_NAME}"
readonly TERRA_EXE="${JUPYTER_USER_HOME}/SageMaker/.terra/bin/terra"
...
TAGS_EXIST=$(jq "has(\"${CLI_SERVER_NAME_TAG_KEY}\") and has(\"${TERRA_WORKSPACE_ID_KEY}\")" /etc/instance-tags )
if [ $TAGS_EXIST == false ]; then
    wait
    echo "Script cannot continue, required tags ${CLI_SERVER_NAME_TAG_KEY} and/or ${TERRA_WORKSPACE_ID_KEY} missing."
    exit 0
fi

# Resolve CLI Server, Workspace ID, and Expected AWS Config File location from tags
TERRA_CLI_SERVER_NAME="$(jq -r .${CLI_SERVER_NAME_TAG_KEY} /etc/instance-tags)"
TERRA_WORKSPACE="$(jq -r .${TERRA_WORKSPACE_ID_KEY} /etc/instance-tags)"
...
export TERRA_CLI_VERSION="${TERRA_CLI_VERSION}"
export TERRA_CLI_SERVER_NAME="${TERRA_CLI_SERVER_NAME}"
...
configure_workspace() {
  ...
  ${TERRA_EXE} workspace configure-aws --cache-with-aws-vault
}

configure_ssh() {
  USER_SSH_DIR="${HOME}/.ssh"
  mkdir -p ${USER_SSH_DIR}
  USER_SSH_KEY=$(${TERRA_EXE} user ssh-key get --include-private-key --format=JSON)
...
TERRA_DIR="${HOME}/SageMaker/.terra"

# Lazy create terra persistence directory on EBS volume to survive restarts and symlink it in
# user's home directory.

echo "Creating ~/SageMaker/.terra directory and ~/.terra symlink"
mkdir -p "${TERRA_DIR}"
ln -sfn "${TERRA_DIR}" "${HOME}/.terra"

# If Terra not already installed, download and install it ... this must be done as ec2-user for
# Terra state to persist for user!

if [ ! -f "${TERRA_EXE}" ]; then
    echo "Downloading and installing Terra CLI version ${TERRA_CLI_VERSION}"
    wget https://github.com/DataBiosphere/terra-cli/releases/download/${TERRA_CLI_VERSION}/terra-cli.tar

    echo "Extracting Terra CLI tarball to '${TERRA_DIR}'."
    tar --extract --file=terra-cli.tar --directory=${TERRA_DIR} --strip-components=1
    rm terra-cli.tar

    echo "Configure Terra CLI for manual login and setting server to ${TERRA_CLI_SERVER_NAME}"
    ${TERRA_EXE} config set browser manual
    ${TERRA_EXE} server set --name ${TERRA_CLI_SERVER_NAME}
fi
