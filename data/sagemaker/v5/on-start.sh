...
configure_ssh() {
  ...
  USER_SSH_KEY=$(${WORKBENCH_EXE} user ssh-key get --include-private-key --format=JSON)
...
readonly VERSION_JSON="$(curl -s "https://${WORKBENCH_CLI_SERVER/verily/terra}-axon.api.verily.com/version")"
do_install_cli=true
