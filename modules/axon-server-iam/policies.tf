# --- assume role policy for axon server ---

data "aws_iam_policy_document" "assume_axon_server" {
  statement {

    # This specifies a federated identity using Google's OIDC Provider, which is trusted by AWS.
    principals {
      type        = "Federated"
    ...
    condition {
      ...
      values   = [var.axon_server_gcp_service_account.aud]
    }

    condition {
      test     = "StringEquals"
      variable = "accounts.google.com:sub"
      values   = [var.axon_server_gcp_service_account.sub]
    }
  }
}
